...
def RANSAC(point_map, threshold=THRESHOLD, verbose=True):
    ...
    for i in range(NUM_ITERS):
        ...
        if (a <= 0):
            ...
            pairs = [point_map[i] for i in np.random.choice(a, 4)]
...
def main(image1, image2, directory, verbose=True):
    ...
    output = util.drawMatches(image1, image2, point_map, inliers)
    
    cv2.imwrite(util.OUTPUT_PATH + 'inlier_matches.png', output)
